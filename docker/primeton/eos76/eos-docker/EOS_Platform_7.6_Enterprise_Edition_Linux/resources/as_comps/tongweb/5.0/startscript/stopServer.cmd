@echo off
rem ---------------------------------------------------------------------------
rem The TongWeb Server Start Script
rem
rem Environment Variable Prequisites
rem
rem   JAVA_HOME       Point at your Java Development Kit installation. If you
rem                   don't want to use the JDK specified by JAVA_HOME, define
rem                   JDKDIR in this script.
rem
rem
rem Configurable Variables in this Script
rem
rem   JDKDIR          Point at your Java Development Kit installation. If not
rem                   specified, JAVA_HOME will be used.
rem
rem   JAVA_OPTS       Additional java arguments.
rem
rem ---------------------------------------------------------------------------
...
set TW_OPTS=%TW_OPTS% -Djavax.net.ssl.keyStore="%TWNS_HOME%\config\server.keystore"
set TW_OPTS=%TW_OPTS% -Djavax.net.ssl.trustStore="%TWNS_HOME%\config\cacerts"
...
set TW_OPTS=%TW_OPTS% -Dtongweb.logFile="%TWNS_HOME%\logs\server.log"
...
set TW_OPTS=%TW_OPTS% -Dtongweb.classloader.sharedChainJars="%TWNS_HOME%\lib\ant\lib\ant.jar"
set TW_OPTS=%TW_OPTS% -Dtongweb.classloader.optionalOverrideableChain=webservices-rt.jar,webservices-tools.jar
set TW_OPTS=%TW_OPTS% -Dtongweb.classloader.excludesList=admin-cli.jar,twns-anttask.jar
...
set TWNS_CLASSPATH=%TWNS_CLASSPATH%;"%TWNS_HOME%\lib\launch.jar";"%TWNS_HOME%\lib\license.jar"

:RESTART
"%JAVA%" %JAVA_OPTS% %TW_OPTS% -classpath %TWNS_CLASSPATH% com.tongweb.server.PELaunch stop %*
